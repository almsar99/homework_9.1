...
Функциональный код проекта покрыт тестами на 100%.
...
В проект добавлен модуль `search`, реализующий работу с регулярными выражениями
и подсчётом категорий банковских операций.

### process_bank_search

Функция выполняет поиск операций по ключевому слову в поле `description`
...
**Возвращает:**
- список транзакций, в описании которых найдено совпадение
...
Функция подсчитывает количество операций по категориям,
...
stats = process_bank_operations(transactions, ["Перевод", "Открытие вклада"])
```

---

## Консольный интерфейс (main)

В проект добавлена функция `main`, объединяющая функциональность проекта
...
- выполнить поиск по слову в описании
